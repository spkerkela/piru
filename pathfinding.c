...
bool path_solid_pieces(PathNode *path, int a, int b) { return true; }
...
PathNode *path_pop_active_step() { return search_stack[--search_stack_size]; }
...
bool path_parent_path(PathNode *path, const Point next_destination,
  ...
  next_g_movement_cost =
      path->g_movement_cost + path_check_equal(path, next_destination);
  ...
  if (*previous_node) {
    while (true) {
      ...
      reconstructed_path[++path_length - 1] =
          path_directions[3 * (current_node->y - (*previous_node)->y) -
                          (*previous_node)->x + 4 + current_node->x];
